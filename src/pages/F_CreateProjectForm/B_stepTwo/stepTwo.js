...
const CreateProjectFormStepTwo = ({navData, errors, removeFromErrorArray, addToErrorArray}) => {
  ...
  const validation = () => {
    console.log(ref1.checked)
    console.log(ref2.checked)
    console.log(ref3.checked)
    if(description !== '' && ref1.checked && ref2.checked && ref3.checked) removeFromErrorArray('');
    else addToErrorArray('description', RESPONSE.NoDescription)
  }
 ...
 //Displays the code quill generates & places empty lines for visibility
  const handleChange = (html) => {
    if(errors.value['description'])removeFromErrorArray('description')
    setDescription(html);
    ...
    <div className={style.wrap}>
        ...
        <p onClick={validation}>CHECK VALIDATION</p>
          ...
          <label className={`${style.midsection__item}`} htmlFor={'budget'}>
            <p className={`${parentStyle.inputTitle}`}>TOGGLE READ ONLY {readOnly}</p>
